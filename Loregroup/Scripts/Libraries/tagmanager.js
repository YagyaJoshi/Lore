...
(function($) {
    ...
    publicMethods = {
        pushTag: function (tag, ignoreEvents, externalTagId) {
            ...
            if (opts.validationData != null) {
                console.log(opts.validationData);
                if (!Desertfire.ItemExistsInArray(opts.validationData, tag)) {
                    console.log("tag is not present in the list I can not add it.");
                    return false;
                }
            }
            var tagObjId = tag.id;
            ...
            if (alreadyInList) {
                ...
                html += '<a href="#" class="tm-tag-remove" id="' + newTagRemoveId + '" TagIdToRemove="' + tagId + '" TagObjId="' + tagObjId + '">';
                ...
                if (opts.tagsContainer !== null) {
                    ...
                    if (tlid.length > 1) {
                        ...
                        if ($self.parent("span.twitter-typeahead").length > 0) {
                            lastTagObj = $self.parent("span.twitter-typeahead").siblings("#" + $self.data("tm_rndid") + "_" + tlid[tlid.length - 2]);
                        }
                        lastTagObj.after($el);
                    } else {
                        if ($self.parent("span.twitter-typeahead").length > 0) {
                            lastTagObj = $self.parent("span.twitter-typeahead").before($el);
                        } else {
                            $self.before($el);
                        }
                    }
                ...
                $el.find("#" + newTagRemoveId).on("click", $self, function(e) {
                    ...
                    //TagObjId
                    var TagObjId = parseInt($(this).attr("TagObjId"));
                    if (typeof opts.onRemoveTag === 'function') {
                        if (opts.onRemoveTag(TagObjId)) {
                            opts.onRemoveTag(TagObjId);
                        }
                    }
                });
